...
各阶段按 scope 收敛：analyze/translate 内存过滤 `procedureOrder`（不改盘上 analysis.json），plan/scaffold/review/dedup/verify 走 metadata + workOrder banner 提示驱动。同包 bare-name 调用（`helper_proc()` 无包前缀）由 `scanBareCallSites` 补边，否则会漏译。
