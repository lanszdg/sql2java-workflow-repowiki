...
**运行时**：本提示词中使用 `wfutil` 工具执行文件操作，调用方式：`wfutil(command="<cmd>", args=[...])`。
...
2. **校验前置 artifact**：检查目标阶段的必需 artifact 是否存在于 `.workflow-artifacts/` 目录下：
   ...
   使用 bash 检查文件存在性：
   ```bash
   wfutil(command="find-json", args=[".workflow-artifacts"])
   ```
...
1. **校验 path**：确认路径存在
   ```
   wfutil(command="exists", args=["<path>"])
   ```
...
1.5 **Schema 预获取**
   ...
   ```
   wfutil(command="exists", args=["<path>/db.xml"])
   ```
...
1.6 **项目结构定义查找**
   ...
   ```
   wfutil(command="exists", args=["<path>/project-structure.md"])
   ```
