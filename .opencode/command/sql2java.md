...
/sql2java <自然语言描述>      # 例如：帮我把 /path/sql 下的存储过程转成 java，配置在 db.xml，主入口是 ORDER_PKG
```
...
| `dbConf`（db.xml 路径） | 否 | 第一步未指定时，在 `path` 目录下自动查找 `db.xml`；都没有则无 db.xml 模式 |
| `specConf`（规约文件） | 否 | 第一步未指定时，在 `path` 下找 `project-spec.md`（旧 `project-structure.md`）；都没有则用内置默认规约和 Maven 结构 |
...
提取规则：
- **路径识别**：文本中的绝对/相对路径（含 `/`、`\`、或带 `.xml`/`.md`/`.sql` 等扩展名）优先匹配为 `path`/`dbConf`/`specConf`。带 `db.xml`/`db_conf` 字样的归 `dbConf`；带 `spec`/`project-spec`/`project-structure` 字样的归 `specConf`；其余目录路径归 `path`。
