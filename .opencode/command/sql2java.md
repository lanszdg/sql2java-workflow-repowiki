...
3. LOOP:
   ...
   b. 读取返回结果：
      ...
      - Repowiki Provider fast path: metadata.dispatch == false && metadata.nextAction == "advance" → Provider 已写入 analyze 产物，不发起 SubtaskPartInput；直接调用 workflow({ action: "advance", runId })，然后按步骤 f 处理 advance 返回结果
      - Repowiki Provider auto-prepare: only when `REPOWIKI_ANALYZE_PROVIDER=1` and `REPOWIKI_AUTO_PREPARE=1` (or equivalent workflow metadata) are set, Provider may call the existing Repowiki runtime to prepare `.repowiki/knowledge/functions.json`; this is not a new skill and does not replace inventory/scan.
      - metadata.dispatch != true → 先看 metadata.nextAction；若为 "advance"/"confirm"/"retry"/"dispatch"/"finished"/"user_decision" 则按下方 nextAction 规则执行；只有无可执行 nextAction 时才视为完成/暂停/异常并退出循环
