...
> **双目录模式**：包头（声明）和包体（实现）在两个目录时，用 `--header <dir> --body <dir>` 替代位置 `<path>`。scanner 保证 header 先于 body 处理，保住 body-only 私有过程。单目录仍用位置 `<path>`（header/body 同根，按包名配对）。
...
| 字段 | 必填 | 缺省规则 |
|------|------|----------|
| `path`（PL/SQL 源码目录） | 条件必填 | `--header`+`--body` 双目录模式可不提供；否则抽不出则**追问用户**"请提供 PL/SQL 源码目录路径"，不自行编造、不继续 |
