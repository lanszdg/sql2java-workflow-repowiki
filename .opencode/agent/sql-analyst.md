...
本提示词中使用 `wfutil` 工具执行文件操作，调用方式：`wfutil(command="<cmd>", args=[...])`。
...
2. 创建目录：
   ```bash
   wfutil(command="mkdir", args=["${artifactsDir}/inventory-packages"])
   ```
...
```bash
# 验证完整性
wfutil(command="count-json", args=["${artifactsDir}/inventory-packages"])
```
...
- 用 bash 检查已完成的包文件：
  ```bash
  wfutil(command="list-json", args=["${artifactsDir}/inventory-packages"])
  ```
...
3. 创建目录结构：
   ```bash
   wfutil(command="mkdir", args=["${artifactsDir}/analysis-packages", "${artifactsDir}/fsd"])
   ```
4. 确定批次计划：将包按 2-3 个一组分批
   ...
   - 但仍需写入 `analysis-packages/{PKG}.json`（`subprograms: []`），**在开始分批前先处理这些包**：
     ...
     wfutil(command="init-analysis-packages", args=["${artifactsDir}/analysis-packages", "PKG_A,PKG_B,PKG_C"])
     ```
...
```bash
wfutil(command="grep-calls", args=["${sourcePath}/pkg"])
```
...
wfutil(command="count-json", args=["${artifactsDir}/analysis-packages"])

# 2. FSD 文件数 vs inventory 子程序总数（逐包对比）
wfutil(command="validate-fsd", args=["${artifactsDir}"])

# 3. 检查是否有 FSD 包含"详见"占位符（应该为 0）
wfutil(command="check-stubs", args=["${artifactsDir}/fsd", "--exit-with-count"])
```
...
- 用 bash 检查已完成的包文件：
  ```bash
  wfutil(command="list-json", args=["${artifactsDir}/analysis-packages"])
  ```
...
- 用 bash 检测"详见"占位符文件：
  ```bash
  wfutil(command="check-stubs", args=["${artifactsDir}/fsd"])
  ```
- 从第一个未完成的包继续分批处理
