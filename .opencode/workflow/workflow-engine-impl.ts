...
import { runRepowikiAnalyzeProviderForDispatch } from "../workflow/repowiki-provider"
      ...
      execute: async (args: any, context: any) => {
        ...
        switch (args.action) {
          ...
          case "dispatch": {
            ...
            const mdForRepowiki = run.metadata as Record<string, unknown>
            const repowikiDispatch = runRepowikiAnalyzeProviderForDispatch({
              currentPhase: run.currentPhase ?? undefined,
              runId,
              artifactsDir,
              targetUnits: currentEntry?.incrementalContext?.targetUnits as string[] | undefined,
              metadata: mdForRepowiki,
              sourcePath: typeof mdForRepowiki.sourcePath === "string" ? mdForRepowiki.sourcePath : undefined,
              l2FactsFile: typeof mdForRepowiki.repowikiL2FactsFile === "string" ? mdForRepowiki.repowikiL2FactsFile : undefined,
              repowikiRoot: typeof mdForRepowiki.repowikiRoot === "string" ? mdForRepowiki.repowikiRoot : undefined,
              shardIndex: currentEntry?.incrementalContext?.shardIndex !== undefined
                ? Number(currentEntry.incrementalContext.shardIndex) + 1
                : undefined,
            })
            if (repowikiDispatch.handled && repowikiDispatch.response) {
              return repowikiDispatch.response
            }
